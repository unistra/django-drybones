...
Pour créer le nouveau projet en utilisant le template::
    ...
    $ django-admin.py startproject --template=https://github.com/unistra/django-drybones/archive/django1.7.zip --extension=html,rst,ini,coveragerc --name=Makefile myapp
...
Vous pouvez ajouter une fonction à vos dotfiles pour faciliter la création d'un projet::
    ...
    initproject() {
        ...
        if [ -z "$1" ];then
            ...
            django-admin.py startproject --template=https://github.com/unistra/django-drybones/archive/django1.7.zip --extension=html,rst,ini,coveragerc --name=Makefile $PROJECT_NAME
            cd $PROJECT_NAME
            setvirtualenvproject $VIRTUAL_ENV $(pwd)
